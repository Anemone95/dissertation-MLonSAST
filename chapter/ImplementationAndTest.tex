...
\subsubsection{评估数据集}
本文使用 OWASP Benchmark v1.1~\footnote{\url{https://www.owasp.org/index.php/Benchmark}} （ 简称 OWASP）和 Juliet Test Suite for Java v1.3~\footnote{\url{https://samate.nist.gov/SARD/testsuite.php}} （ 简称 Juliet）作为数据集，由于 Juliet 数据集存在大量多个入口点共用一个汇聚点的漏洞情况，导致大部分漏洞均为正报（因为只要漏洞实例中任意一棵传播树是不安全的，那么漏洞就是真实存在的），从而无法在 RQ1 中使用，其将在 RQ2 和真实系统上线时，作为切片训练集使用，表~\ref{tab:dataset} 中显示了各个数据集中的数据量大小。
