...
% 南大中文封面内容字符串
\title{基于污点分析和BLSTM的Java代码扫描系统的设计和实现}
\author{徐文远}
...
\begin{abstract}
随着软件行业规模不断扩大，对于软件和信息系统的攻击日益增多，人们在快速开发软件的同时也越来越关注软件质量。为此，工程师在软件开发和测试过程中对使用静态代码分析系统对软件代码进行安全性扫描，并对报告进行评估，只有通过安全性评估的软件才可以部署至线上运行。然而，代码分析往往是保守的，为了不遗漏安全风险，系统往往会抛出大量的误报，这些误报增加了安全工程师的工作量，进而对软件开发进度产生影响。随着机器学习领域的发展，学术界已有将机器学习算法运用于代码分析以发现漏洞或降低误报的前沿性研究，然而他们的工作只适用于小规模程序，对于在大规模应用还存在着种种问题。

本系统旨在将学术研究成果代入工业界实际应用，面向Web开发常用的Java语言，利用程序切片和BLSTM提升污点传播准确性，减轻安全工程师的工作量，保证软件的开发进度和质量。首先，在污点传播方面，本系统利用find-sec-bug的大量规则，保证低漏报的同时，对该工具的输出进行改进，使之能反馈更详细的污点传播树；接着，本系统利用程序切片技术，对每一个漏洞实例进行代码切片，为了保证效率，本文提出分段切片思想，即将污点传播树分解为一个个小的污染流片段，再对片段切片；最后，系统通过BLSTM模型，对漏洞的切片集合做预测，根据片段的预测结果预测漏洞本身是否为误报。

本系统目前已代替传统的污点传播分析引擎在线上部署，同时实验结果表明，本系统可以在不耽误较多扫描时间的情况下，取得更加准确的污点扫描结果。在效率上，本系统优化了传统切片，每个项目的整体扫描时间不超过 1 小时，在准确性上，本系统的误报预测精确率达 90.53 \%，即相对于单纯的污点传播，本系统在遗漏少量真实漏洞的前提下，排除了 25.44 \% 误报，大大减轻了安全运营人员的代码审计工作量，从而在整体上加快了软件开发过程。
